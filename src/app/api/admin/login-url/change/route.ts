...
export async function POST(req: NextRequest) {
  ...
  try {
    const { code, newSlug } = (await req.json()) as { code?: string; newSlug?: string };
    if (typeof code !== 'string' || typeof newSlug !== 'string') {
    ...
    const otpResult = await verifyOtp(session.email, 'change_admin_login_url', code);
    if (!otpResult.valid) {
